...
import type React from "react" // Not strictly necessary if not using React.FC or similar
...
import { ChevronLeft, ChevronRight, Download, MapPin, ExternalLink } from "lucide-react" // Added MapPin, ExternalLink
import Link from "next/link"
import { ZoomableImage } from "./ui/zoomable-image" // Assuming this path is correct and component exists
...
// Property data - Using the evocative names, ensure all image arrays are complete
const propertyTypes: PropertyBlock[] = [
  {
    ...
    types: [
      ...
      },
      {
        id: "a-2-1",
        name: "A 2+1",
        displayName: "The Expansive Home", // Or "The Horizon Family Home"
...
export default function ResidencesSection() {
  ...
  const formatResidenceCardDetails = (details: PropertyDetail): string => {
    if (details.bedrooms === 1 && details.livingRooms === 0) { // Typical Studio definition
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-h-[95vh] w-[95vw] max-w-4xl overflow-y-auto overflow-x-hidden p-0 data-[state=open]:animate-contentShow sm:rounded-lg md:w-full">
         {/* FIX: Wrap the conditional content in a single, always-present div */}
              ...
              <>
                ...
                <div className="p-5 sm:p-6 md:p-8">
                  ...
                  <div className="grid grid-cols-1 gap-x-6 gap-y-5 md:grid-cols-2 md:gap-x-8 md:gap-y-6">
                    <div>
                      <h4 className="mb-2.5 text-lg font-semibold text-[#1a1a1a] md:mb-3">Details</h4>
                    ...
                    <div>
                      <h4 className="mb-2.5 text-lg font-semibold text-[#1a1a1a] md:mb-3">Features</h4>
                      <ul className="space-y-2 text-sm md:space-y-2.5">
          ...
          </div> {/* End of the new wrapper div for DialogContent children */}
        </DialogContent>
      </Dialog>
    </section>
  );
